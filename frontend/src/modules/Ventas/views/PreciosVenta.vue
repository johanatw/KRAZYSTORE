...
onMounted(() => {
    ...
    PreciosVentaService.getPreciosProducto(router.currentRoute.value.params.id).then((data) => {
...
const modificarProducto = (id) => {
    PreciosVentaService.getPrecio(id).then((data) => {
...
const saveProducto = () => {
    ...
    if (precio?.value.fecha && precio?.value.precio) {
        if (precio.value.id) {
          console.log("proveedor.value");
          console.log(precio.value);
          PreciosVentaService.modificarPrecio(precio.value.id, precio.value).then((response)=>{
...
<template>
  ...
  <div class="flex p-fluid justify-content-center " >
    ...
    <Dialog v-model:visible="precioDialog" :closable="false" :style="{width: '450px'}" header="Precio de venta" :modal="true" class="p-fluid">
        <div class="formgrid">
        <div class="field">
        ...
        <div class="field">
            ...
            <InputNumber fluid id="description" v-model="precio.precio" required="true" :invalid="submitted && !precio.precio"  />
            <small class="p-error" v-if="submitted && !precio.precio">Ingrese el precio</small>
        </div>
    </div>
        <template #footer>
