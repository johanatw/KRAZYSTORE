...
const tipoPedido = ref({cod: 'C', descripcion:'Compra'});
...
async function guardarAnticipo() {
    ...
    let ant = {total: sumaTotal.value, fecha: fechaAnticipo, pedidoCompra: pedido.value};

    let anticipoCreationDTO = {pagoPedidoCompra: ant, pagos: pagos.value};
    console.log(anticipoCreationDTO);
    CajaServices.savePagoPedidoCompra(anticipoCreationDTO ).then((data)=> {
        getAnticipos();
        closeDialog();
...
const guardarReembolso = () =>{
    if (cajaAbierta.value != null) {
        ...
        fecha: fecha,
        motivo: motivo.value,
        pagoPedidoCompra: anticipo.value};
        
        let reembolsoCreationDTO = {reembolso: reembolso, cobros: pagos.value};
        CajaServices.saveReembolsoPagoPedidoCompra(reembolsoCreationDTO).then((data)=> {
...
const deleteAnticipo = (id) =>{
    ...
    CajaServices.deletePagoPedidoCompra(id).then((response)=>{
    ...
    <div class="card flex p-fluid justify-content-center " >
        ...
        <template #container="{ message, acceptCallback }">
            ...
            <Dialog v-model:visible="visible" modal @update:visible="closeDialog($event)" header="Edit Profile" :style="{ width: '30rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
                <template #header>
                    <div class="flex align-items-center gap-2">
                        <h3 class="font-bold">Reembolsar Pago</h3>
                    </div>
                </template>
                        ...
                        <div class="field col-12 md:col-12">
                            <div class="field col-12 md:col-12">
                                <Card style="height: 100%;" >
                                    <template #title> Detalle del Pago </template>
                                    <template #content>
                                        <div >
                                            <label for="fecha" style="font-weight: 600;" >Fecha:</label>
                                            {{ formatearFechaHora(anticipo.fecha) }}<br>
                                            <label for="fecha" style="font-weight: 600;" >Monto total:</label>
                                        ...
                                        <div>
                                            <div class="formgrid grid" v-for="item, index in pagos" :key="item" >
                                                <div class="field col-12 md:col-6" style="justify-content: start;  ">
                                                    <Select class="flex flex-grow-1" style="padding: 0rem !important;" v-model="item.medio" :options="formasPago" @change="habilitarInput(index, item)" optionLabel="descripcion" placeholder="Seleccione un elemento"   /> 
                                                </div>
                                ...
                                <template #content>
                                    <div class="formgrid grid">
                                        <div class="flex field col-12 md:col-6 "style="height: 1.5rem;" >
                                            <label for="email" class="w-9rem" style="font-weight: 600;" >Pedido de Compra N°</label>
                                        </div>
                                        <div class="flex field col-12 md:col-6 " style="height: 1.5rem;">
                                            <InputGroup>
                                        ...
                                        <div class="flex field col-12 md:col-6 " style="height: 1.5rem;">
                                            <label  style="font-weight: 600;" >Total pagado:</label>
                                        </div>
                                        <div class="flex field col-12 md:col-6 " style="height: 1.5rem;">
        ...
        <Panel style=" position: relative; width: 100%;" >
            <template #header>
                <div class="flex align-items-center gap-2">
                    <h3 class="font-bold">Anticipos Proveedores</h3>
                </div>
            </template>
            ...
            <div >
                ...
                <template #loading> Cargando. </template>
                    ...
                    <Column :exportable="false" style="min-width:8rem">
                        <template #body="slotProps">
                            <Button severity="success" icon="pi pi-sync" v-tooltip="'Reembolsar'" text rounded aria-label="Cancel" @click="showDialogReembolso(slotProps.data)"  style="height: 2rem !important; width: 2rem !important;" />
