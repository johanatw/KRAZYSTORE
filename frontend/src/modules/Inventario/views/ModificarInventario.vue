  ...
  <template #icons>
    ...
    <div >
      ...
      <MultiSelect
        ...
        id="categoria-filter"
        v-model="filters['producto.categoria'].value"
        :options="categorias"
         ...
         <Column  class="col" field="diferencia" header="Diferencia" aria-sort="none" >
             <template #body="slotProps">
                 <div class="flex-auto p-fluid" style="max-width: 20dvh;">
                     <label for="diferencia"> {{  (slotProps.data.diferencia =  slotProps.data.cantContada - slotProps.data.cantStock).toLocaleString("de-DE") }}</label>
                  </div>
            </template>
         </Column>
        </DataTable>
...
onMounted(() => {
    InventarioServices.obtenerDetallesCompletos(router.currentRoute.value.params.id).then((data) => {
       ...
       detalleInventario.value = data.data.detalle;
        ...
        filters.value['producto.categoria'].value = filtros.value;
    });
